...
public class ContextView extends VerticalLayout {
	...
	private void init() throws EscidocClientException {
		...
		final DirectMember directMembers = new DirectMember(mainSite, resourceProxy.getId());
		leftCell(directMembers.contextasTree());

		//Right Inner Cell
		// Binding Additional Info into it
		ContextAddInfo cnxAddinfo = new ContextAddInfo(resourceProxy,appHeight);
	...
	/**
	 * This is the inner Right Cell within a Context
	 * By default a set of Organizational Unit / Admin Description / RelatedItem / Resources are bound
	 * @param comptoBind
	 */
	...
	/**
	 * This is the inner Left Cell within a Context
	 * By default the Direct Members are bound here
	 * @param comptoBind
	 */
	...
	/**
	 * Bindind Context Properties
	 * 2 sets of labels in 2 rows
	 */
	private void bindProperties(){
		//LEFT SIde
		final Label descMetadata1 = new Label(NAME + resourceProxy.getName()
				+ " <br /> " + DESCRIPTION + resourceProxy.getDescription()
				+ "<br />" + "ID: " + resourceProxy.getId() + " is "
		...
		final Label descMetadata2 =
			new Label(
					CREATED_BY+"<a href='/ESCD/Frankie'>"+resourceProxy.getCreator()+"</a>"+resourceProxy.getCreatedOn()+"<br>"
					+ LAST_MODIFIED_BY+ " <a href='#user/"+resourceProxy.getModifier()+"'>"+resourceProxy.getModifier()+"</a>"+resourceProxy.getModifiedOn()+" <br>",Label.CONTENT_RAW);
