...
public class SearchResultsView extends VerticalLayout {
    ...
    public SearchResultsView(final Router router, LayoutDesign layout, final String searchString,
        ...
        try {
            ...
            router.getApplication().getMainWindow().showNotification(e.getMessage());
        }
        addComponent(cssLayout);
    }

    public SearchResultsView(final Router mainSite, final EscidocServiceLocation serviceLocation,
    ...
    private PagedTable createPagedTable(final CssLayout cssLayout) {
        ...
        tblPagedResults.addListener(new Property.ValueChangeListener() {
            ...
            private void openSelectedSearchResult(final Object[] variablesForTheTab) {
                router.openTab(buildResourceView(variablesForTheTab), getTabName(variablesForTheTab));
            }
                ...
                if (variablesForTheTab[0].equals(ResourceType.CONTAINER.asLabel())) {
                    try {
                        return new ContainerView(serviceLocation, router, find(variablesForTheTab,
                            ResourceType.CONTAINER), router.getWindow(), router.getCurrentUser(), repositories);
                    }
                    catch (final EscidocClientException e) {
                        showerror();
                    }
                    ...
                    try {
                        return new ContextView(serviceLocation, router, find(variablesForTheTab, ResourceType.CONTEXT),
                            router.getWindow(), router.getCurrentUser(), repositories);
                    }
                    catch (final EscidocClientException e) {
                        showerror();
                    }
    ...
    public void onClick(final Button.ClickEvent event) {
        final SearchAdvancedView srch = new SearchAdvancedView(router, serviceLocation);
