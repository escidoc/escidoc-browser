...
public class SearchResultsView extends VerticalLayout {
    ...
    public SearchResultsView(final Router mainSite, final String searchString,
        ...
        final SearchRepositoryImpl srRep =
            new SearchRepositoryImpl(serviceLocation);
    ...
    public SearchResultsView(final Router mainSite,
        final EscidocServiceLocation serviceLocation, final String titleTxt,
        final String creatorTxt, final String descriptionTxt,
        final Object creationDateTxt, final String mimesTxt,
        final String resourceTxt, final String fulltxtTxt) {
        this.router = mainSite;
        this.serviceLocation = serviceLocation;
        final CssLayout cssLayout = configureLayout();
        final SearchRepositoryImpl srRep =
            new SearchRepositoryImpl(serviceLocation);
        results =
            srRep.advancedSearch(titleTxt, creatorTxt, descriptionTxt,
    ...
    private void createPaginationTblResults(final CssLayout cssLayout)
        ...
        for (final SearchResultRecord record : records) {
            ...
            if (s.getContent() instanceof Container) {
                resourceProxy =
                    new ContainerProxyImpl((Container) s.getContent());
                strResourceType = "Container";
            }
            else if (s.getContent() instanceof de.escidoc.core.resources.om.item.Item) {
                final Resource item =
                    (de.escidoc.core.resources.om.item.Item) s.getContent();
                resourceProxy =
                    new ItemProxyImpl(
            ...
            final Object[] variablesForTheTab =
                { strResourceType, resourceProxy.getId(), resourceProxy.getId() };
            ...
            item.getItemProperty("Type").setValue(
                new Label("<img src= \"/browser/VAADIN/themes/myTheme/images/"
                    + strResourceType + ".png\" />", Label.CONTENT_RAW));
        ...
        final Label lblResults =
            new Label("We found " + results.getNumberOfMatchingRecords()
    ...
    private PagedTable createPagedTable(final CssLayout cssLayout) {
        ...
        tblPagedResults.addListener(new Property.ValueChangeListener() {
            ...
            public void valueChange(final ValueChangeEvent event) {
                final Object[] variablesForTheTab =
                    (Object[]) tblPagedResults.getValue();
            ...
            private Component buildResourceView(
                ...
                if (variablesForTheTab[0].equals(ResourceType.CONTAINER
                    .asLabel())) {
                    try {
                        return new ContainerView(serviceLocation, router, find(
                            variablesForTheTab, ResourceType.CONTAINER), router
                            .getWindow(), router.getCurrentUser(), repositories);
                    }
                    catch (final EscidocClientException e) {
                        showerror();
                    }
                    ...
                    .asLabel())) {
                    try {
                        return new ItemView(serviceLocation, repositories,
                            router,
                            find(variablesForTheTab, ResourceType.ITEM), router
                                .getWindow(), currentUser);
                    }
                    catch (final EscidocClientException e) {
                        showerror();
                    }
                    ...
                    .asLabel())) {
                    try {
                        return new ContextView(serviceLocation, router, find(
                            variablesForTheTab, ResourceType.CONTEXT), router
                            .getWindow(), router.getCurrentUser(), repositories);
                    }
                    catch (final EscidocClientException e) {
                        showerror();
            ...
            private ResourceProxy find(
                final Object[] variablesForTheTab,
                final ResourceType resourceType) throws EscidocClientException {
    ...
    public void onClick(final Button.ClickEvent event) {
        final SearchAdvancedView srch =
            new SearchAdvancedView(router, serviceLocation);
